...
Реализация через кеш для примера. Наилучшее решение для этой задачи будет прямое обновление в БД. При этом если будет лишняя нагрузка на основную БД, то можно вынести в микросервис. 
Запись кэшированных данных в БД с помощью CLI-команды по расписанию с блокировкой. Блокировку можно будет убрать когда в клиенте redis для PHP будет реализована команда `GETDEL`.  
Обновление счетчика выполняется без блокировки, но при этом с ожиданием разблокировки записи из кеш в БД.
